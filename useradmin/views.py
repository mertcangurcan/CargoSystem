...
def index(request):
	if 'userid' in request.COOKIES and User.objects.filter(id=int(request.COOKIES['userid']))[0].isAdmin == False:
		return render_to_response("home/home.html", {'userid':request.COOKIES['userid'], 'message':'Invalid operation.', 'messagetype':2})
	elif 'userid' not in request.COOKIES:
		return render_to_response("home/home.html", {'message':'Invalid operation.', 'messagetype':2})
